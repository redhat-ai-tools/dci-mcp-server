...
def register_job_tools(mcp: FastMCP) -> None:
    ...
    async def search_dci_jobs(
        ...
        The is also a `in` operator to check if a value is in a list of values. The `not_in` operator checks if a value is not in a list of values. The `and`, `or`, and `not` operators are supported to combine multiple criteria. Parentheses can be used to group criteria. For example, to get daily jobs with no `debug` tag, use ((tags in ['daily']) and (tags not_in ['debug'])).
        ...
        - created_at: The creation timestamp. Use `today` tool to compute relative dates.  Use the >, <, >=, <= operators to filter jobs by last update date. Do not use the = operator with a date on this field as it means the hour is 00:00:00 UTC.
        ...
        - jobstates
        - keys_values.(key, value): metric associated with the job
        ...
        - updated_at: The last update timestamp. Use `today` tool to compute relative dates. Use the >, <, >=, <= operators to filter jobs by last update date. Do not use the = operator with a date on this field as it means the hour is 00:00:00 UTC.
